...
class Augmentor(nn.Module):
    ...
    @staticmethod
    def __inplace_augment_helper(target_concentration: torch.Tensor, alpha: torch.Tensor, beta: torch.Tensor):
    ...
    @staticmethod
    def augment(target_concentration: torch.Tensor,
                target_stain_idx: Optional[Sequence[int]],
                inplace: bool,
                rng: torch.Generator,
                sigma_alpha: float,
        ...
        Args:
            target_concentration: concentration matrix of input image
    ...
    def forward(self, target: torch.Tensor, regularizer: float = 0.01, **stain_mat_kwargs):
        # stain_matrix_target -- B x num_stain x num_input_color_channel
        # todo cache
        target_stain_matrix = self.get_stain_matrix(target, **stain_mat_kwargs)

        #  B x num_stains x num_pixel_in_mask
        concentration = get_concentrations(target, target_stain_matrix, regularizer=regularizer,
