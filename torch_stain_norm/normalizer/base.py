...
from torch_stain_norm.functional.optimization.dict_learning import get_concentrations
from torch_stain_norm.functional.stain_extraction.factory import build_from_name
...
class StainSeperation(Normalizer):
    ...
    def __init__(self, get_stain_matrix, reconst_method):
        """
        Args:
            get_stain_matrix:
            reconst_method:  Sparse encoding to get stain concentration
        """
        super().__init__()
    ...
    @classmethod
    def build(cls, algo: str, *args, **kwargs):
        algo = algo.lower()
        extractor = build_from_name(algo)
